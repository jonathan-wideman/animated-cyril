...
class Main
    gamestate:
        ...
        update: ->
            ...
            @player.handleClick @map.getTile(x, y)
        ...
        getStatusText: ->
            ...
            status += '\n'
            status += 'TOOL: ' + @player.getTool().name + '\n'
            status += @player.getTool().getStatusText() + '\n'
            status += '\n'
            status += 'STOCKPILE: \n'
    ...
    start: =>
        @game = new Phaser.Game(640, 640, Phaser.CANVAS, 'game-container', @gamestate)
